...
use common::{
  column_identifier_no_alias, parse_comment, sql_identifier, statement_terminator, table_reference,
  type_identifier, ws_sep_comma, Literal, Real, SqlType, TableKey,
};
...
pub fn creation(i: &[u8]) -> IResult<&[u8], CreateTableStatement> {
      ...
      ks.into_iter()
        .map(|key| {
          ...
          match key {
            ...
            TableKey::UniqueKey(name, columns) => TableKey::UniqueKey(name, attach_names(columns)),
  ...
  Ok((remaining_input, CreateTableStatement {
    table,
    fields,
    keys,
  }))
...
pub fn view_creation(i: &[u8]) -> IResult<&[u8], CreateViewStatement> {
  ...
  Ok((remaining_input, CreateViewStatement {
    name,
    fields,
    definition,
  }))
...
mod tests {
  ...
  fn simple_create() {
    ...
    assert_eq!(res.unwrap().1, CreateTableStatement {
 ...
 iw_trans tinyint NOT NULL default 0
  ...
  fn keys() {
    ...
    assert_eq!(res.unwrap().1, CreateTableStatement {
      ...
      keys: Some(vec![TableKey::UniqueKey(Some(String::from("id_k")), vec![
  ...
  fn lobsters_indexes() {
    ...
    assert_eq!(res.unwrap().1, CreateTableStatement {
      ...
      keys:   Some(vec![
        TableKey::FulltextKey(Some("index_comments_on_comment".into()), vec![
          Column::from("comments.comment")
        ]),
        TableKey::Key("confidence_idx".into(), vec![Column::from(
          "comments.confidence"
        )]),
        TableKey::UniqueKey(Some("short_id".into()), vec![Column::from(
          "comments.short_id"
        )]),
        TableKey::Key("story_id_short_id".into(), vec![
          Column::from("comments.story_id"),
          Column::from("comments.short_id")
        ]),
        TableKey::Key("thread_id".into(), vec![Column::from("comments.thread_id")]),
        TableKey::Key("index_comments_on_user_id".into(), vec![Column::from(
          "comments.user_id"
        )]),
      ]),
    });
