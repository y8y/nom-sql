...
fn simple_expr(i: &[u8]) -> IResult<&[u8], ConditionExpression> {
  alt((
    ...
    map(
      ...
      |e| ConditionExpression::Bracketed(Box::new(ConditionExpression::Arithmetic(Box::new(e)))),
    ),
    map(literal, |lit| {
...
mod tests {
  ...
  fn flat_condition_tree(op: Operator, l: ConditionBase, r: ConditionBase) -> ConditionExpression {
    ConditionExpression::ComparisonOp(ConditionTree {
      operator: op,
  ...
  fn condition_expression_with_arithmetics_and_parenthesis() {
    ...
    assert_eq!(
      res.unwrap().1,
      ConditionExpression::Bracketed(Box::new(ConditionExpression::ComparisonOp(ConditionTree {
        operator: Operator::Equal,
